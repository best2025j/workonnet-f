...
import { JOB_APPLICATION_STATUS, type IJobApplications } from '~/types';

defineProps<{ jobAppList: IJobApplications[] }>();

const getApplicationStatus = (status: JOB_APPLICATION_STATUS) => {
  if (status === JOB_APPLICATION_STATUS.ACCEPTED) {
    return 'Offered';
  } else if (status === JOB_APPLICATION_STATUS.PENDING) {
    return 'Applied';
  }
  return status;
};
...
<template>
  <div class="py-8 gap-4 grid md:grid-cols-2">
    <div
      v-for="(jobApp, index) in jobAppList"
      :key="index"
    ...
    >
      <div class="flex items-center justify-between border-b-2 p-3">
        <div class="flex items-center space-x-2 md:space-x-4">
          <img
            :src="jobApp.recruiter?.photo?.url"
            alt="Google"
            class="md:w-10 md:h-10 h-8 w-8 rounded"
          />
          <div class="md:pl-4 pl-2">
            <span class="block md:text-base text-xs font-black capitalize">{{
              jobApp.jobListing.title
            }}</span>
            <span
              class="md:text-xs text-[10px] font-semimedium text-gray-500"
              >{{ jobApp.recruiter?.companyName }}</span
            >
          </div>
        </div>
        ...
        <div class="flex items-center md:space-x-6 space-x-2">
          <div class="">
            ...
            <div class="flex space-x-2 items-center mt-2 p-2">
              ...
              <span class="text-xs text-gray-700"
                >NGN{{
                  formatCurrency(jobApp?.jobListing.expectedSalary as number)
                }}
                per month</span
              >
            </div>
            ...
            <div class="flex space-x-2 items-center mt-2 p-2">
              ...
              <span class="text-xs text-gray-700"
                >Posted
                {{
                  formatTimeDiffHuman(jobApp?.jobListing?.createdAt as string)
                }}</span
              >
            </div>
          </div>
