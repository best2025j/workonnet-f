...
<template>
  <div class="w-1/2 mx-auto h-full">
    <div class="flex flex-col items-center justify-center w-[23.375rem] mx-auto mt-32">
      <span class="mt-5">
      ...
      <p class="font-thin text-[12px]">We sent a code to {{ route.query.email }}</p>
      <div class="flex items-center justify-center gap-2 mb-8 mt-8">
        <div class="flex items-center justify-center space-y-6">
          <v-otp-input v-model="formData.code" @change="v$.code.$touch" ref="otpInput" input-classes="otp-input"
            separator="&nbsp;" :num-inputs="5" :should-auto-focus="true" :is-input-num="true"
            :conditionalClass="['one', 'two', 'three', 'four', 'five']"
            :placeholder="['0', '0', '0', '0', '0']" @on-change="handleOnChange"
      ...
      <BtnPrimary @click="verifyCode()" :disabled="isLoading || v$.$invalid" :isLoading="isLoading">
        <template #text>
